...
Agent_Architecture:
  core_components:
    reasoning_engine:
      ...
      models:
        frontier: ["Claude 4 Opus (best reasoning)", "GPT-5 (broad capability)", "o3 (complex planning)"]
        open_weight_agent_models:
          hermes_3:
            provider: "Nous Research"
            base: "Fine-tuned on Llama 3 / Mistral architectures"
            sizes: ["8B", "70B", "405B"]
            strengths:
              - "Purpose-built for agentic use — best open-weight function calling"
              - "Structured JSON output (near-perfect schema adherence)"
              - "Multi-turn tool use with maintained context"
              - "System prompt following (role-play, constraints, boundaries)"
              - "ChatML format with dedicated tool-call tokens"
            training: "Trained on synthetic function-calling datasets + DPO alignment"
            use_case: "Self-hosted agents needing reliable tool use without API dependency"
            comparison: "Matches GPT-4o function-calling quality at 70B; 8B version viable for simple agents"
          functionary:
            provider: "MeetKai"
            what: "Function-calling specialized (trained on OpenAI function-calling format)"
          gorilla:
            provider: "UC Berkeley"
            what: "API-calling specialized (trained on massive API documentation)"
  ...
  agent_specialized_models:
    why: "Not all agents need frontier API models — self-hosted agent models reduce cost and latency"
    
    hermes_3_deep_dive:
      provider: "Nous Research"
      what: "The leading open-weight model family purpose-built for AI agents"
      architecture: "Fine-tuned Llama 3 (8B/70B/405B) with agent-specific training"
      key_features:
        function_calling:
          format: "ChatML with <tool_call> and <tool_response> special tokens"
          reliability: "95%+ valid JSON function calls (matches GPT-4o level)"
          parallel: "Supports parallel function calling (multiple tools in one turn)"
          schema: "Strict JSON Schema adherence for parameters"
        system_prompt_following:
          what: "Exceptional at following complex system prompts"
          enables: "Role-play, personality constraints, output format rules, safety boundaries"
          comparison: "Significantly better than base Llama at respecting system-level instructions"
        structured_output:
          what: "Reliable JSON/YAML/XML generation matching provided schemas"
          use: "API responses, database entries, form filling, data extraction"
        multi_turn_tool_use:
          what: "Maintains coherent tool-use plans across many conversation turns"
          behavior: "Remembers previous tool results, builds on them, handles errors gracefully"
      training_methodology:
        data: "Synthetic function-calling datasets generated by frontier models"
        alignment: "DPO (Direct Preference Optimization) for helpfulness + tool-use quality"
        format: "ChatML template with explicit tool sections"
        innovation: "Trained to reason about WHEN to use tools (not just how)"
      deployment:
        serving: "vLLM, TGI, llama.cpp (GGUF quantizations available)"
        quantization: "INT4 recommended for production (AWQ/GPTQ)"
        hardware:
          hermes_8b: "Single L4/T4 GPU (or CPU with GGUF INT4)"
          hermes_70b: "2× H100 or 4× A100 (INT4: single H100)"
          hermes_405b: "8× H100 (INT4: 4× H100)"
      when_to_use:
        - "Self-hosted agents (data privacy, no API dependency)"
        - "High-volume agent workloads (cost: $0.001/call vs $0.03/call API)"
        - "Custom tool-calling formats (can fine-tune further on your schema)"
        - "Low-latency tool selection (local inference: 20-50ms vs 500ms+ API)"
        - "Air-gapped environments (no internet access for API calls)"
      comparison_vs_alternatives:
        vs_base_llama: "10× better function calling, 5× better schema adherence"
        vs_gpt4o_api: "95% quality, 30× cheaper at volume, no data leaves infra"
        vs_functionary: "Broader capabilities (not just functions — also reasoning, planning)"
        vs_gorilla: "Better general agent behavior (Gorilla is API-calling specialist only)"
      community:
        huggingface: "NousResearch/Hermes-3-Llama-3.1-8B (and 70B/405B variants)"
        fine_tunes: "Active community creating domain-specific Hermes variants"
        ecosystem: "Works with LangChain, LangGraph, CrewAI via standard chat interfaces"
```
